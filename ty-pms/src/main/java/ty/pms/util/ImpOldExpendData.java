				...
				for (int i = 0; i < temp.length; i++) {
					if (!temp[i].equals("")) {
						if (oneExp.getRemark() == null) {
							...
							if ((temp[i].contains("t") || temp[i].contains("tao") || temp[i].contains("涛"))
									&& (!temp[i].contains("y") || !temp[i].contains("yanr") || !temp[i].contains("yan")
											|| !temp[i].contains("燕"))) {
								oneExp.setOwner("涛");
							} else if ((temp[i].contains("y") || temp[i].contains("yan") || temp[i].contains("yanr")
									|| temp[i].contains("燕"))
									&& (!temp[i].contains("t") || !temp[i].contains("tao") || !temp[i].contains("涛"))) {
								oneExp.setOwner("燕");
							} else {
								oneExp.setOwner("涛燕");
							}
						} else if (oneExp.getExpenditureAmount() == null) { // 设置钱数
							if (temp[i].startsWith("+")) { // 收入
								...
								oneIncome.setOwner(oneExp.getOwner());
							} else { // 支出
